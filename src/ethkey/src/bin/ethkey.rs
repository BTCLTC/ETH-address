...
use docopt::Docopt;
use ethkey::{
    sign, verify_address, verify_public, Address, Brain, Error as EthkeyError, Generator, KeyPair,
    Message, Prefix, Public, Random, Secret, Signature,
};
    ...
    S: AsRef<str>,
{
    ...
    return if args.cmd_info {
        ...
        let secret = Secret::from_str(&args.arg_secret)?;
        ...
        let secret = Secret::from_str(&args.arg_secret)?;
        let message = Message::from_str(&args.arg_message)?;
        let signature = sign(&secret, &message)?;
        Ok(format!("{}", signature))
    } else if args.cmd_verify {
        let signature = Signature::from_str(&args.arg_signature)?;
        let message = Message::from_str(&args.arg_message)?;
        let ok = if args.cmd_public {
            let public = Public::from_str(&args.arg_public)?;
            verify_public(&public, &signature, &message)?
        } else if args.cmd_address {
            let address = Address::from_str(&args.arg_address)?;
...
address: 26d1ec50b4e62c1d1a40d16e7cacc6a6580757d5".to_owned();
    ...
    fn secret() {
        ...
        let expected =
            "17d08f5fe8c77af811caa0c9a187e668ce3b74a99acc3f6d976f075fa8e0be55".to_owned();
        assert_eq!(execute(command).unwrap(), expected);
    }
